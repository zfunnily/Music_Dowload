...
class Music_Dowload(QObject):
    ...
    def all_download(self, song_id, music, name, album=None):
        try:
            song_url = f"http://music.163.com/song/media/outer/url?id={song_id}.mp3"
            Music_Funciton.music_download(song_url, music, name, album)
        except:
            pass
    ...
    def all_music_download(self):
        ...
        for i in range(method):
            ...
            music_a = self.ui.music_list_tableWidget.item(i, 2).text()
            music_name.append(music_n)
            music_singe.append(music_s)
            music_album.append(music_a)

        def dodownload():
            self.ongoing = True
            for i in range(method):
                self.all_download(id_info[i], music_name[i], music_singe[i], music_album[i])
